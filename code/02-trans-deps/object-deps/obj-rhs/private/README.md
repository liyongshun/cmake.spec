...
```
 ...
 Properties for TARGET foo:
   ...
   foo.INTERFACE_LINK_LIBRARIES = <NOTFOUND>
...
```
$ cc -DINTERFACE_BAR=2 -DPUBLIC_BAR=1 -Dfoo_EXPORTS -Ifoo/include -Ifoo/src -Ibar/include -fPIC -std=c99 -o build/foo/src/foo.c.o   -c foo/src/foo.c
$ cc -fPIC -shared -Wl,-soname,libfoo.so -o build/foo/libfoo.so build/foo/src/foo.c.o build/bar/src/bar.c.o
```

```
$ cc -Ifoo/include -std=c99 -o build/main/main.c.o -c main/main.c
